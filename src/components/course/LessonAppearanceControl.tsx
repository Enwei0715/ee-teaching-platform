...
export default function LessonAppearanceControl({ appearance, onUpdate }: LessonAppearanceControlProps) {
    ...
    useEffect(() => {
        ...
        window.addEventListener('open-appearance-settings', handleOpen);
        return () => window.removeEventListener('open-appearance-settings', handleOpen);
    }, []);
    ...
    return createPortal(
        <div className="fixed bottom-24 right-6 z-[290] md:right-24 md:bottom-24" ref={menuRef}>
            {/* Panel */}
            <div className="absolute bottom-0 right-0 w-72 p-4 rounded-xl border border-white/10 bg-slate-950/95 backdrop-blur-xl shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-200">
