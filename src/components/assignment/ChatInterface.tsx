                    ...
                    <>
                        {messages.map((msg, idx) => (
                            <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                ...
                                <div className={`text-sm p-3 rounded-lg max-w-[85%] [overflow-wrap:anywhere] [word-break:break-word] whitespace-pre-wrap ${msg.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-800 prose prose-sm max-w-none'}`}>
                                    {msg.role === 'user' ? (
                                        msg.content
                                    ) : (
                                        <ReactMarkdown
                                            ...
                                            components={{
                                                a: ({ node, href, children, ...props }) => {
                                                    if (href && (href.includes('eemaster.enwei.org') || href.startsWith('/'))) {
                                                        const targetUrl = href.includes('eemaster.enwei.org')
                                                            ? `/blog/${href.split('eemaster.enwei.org')[1].replace(/^\//, '') || 'eemaster.enwei.org'}`
                                                            : href;

                                                        // Special case: if the link is JUST the domain, go to the specific blog post requested
                                                        // "Display text" -> http://localhost:3000/blog/eemaster.enwei.org
                                                        const finalUrl = href === 'eemaster.enwei.org' || href === 'https://eemaster.enwei.org'
                                                            ? '/blog/eemaster.enwei.org'
                                                            : targetUrl;
            ...
            <div className="p-4 border-t border-gray-200 bg-gray-50">
                <div className="flex gap-2 mb-2">
                    <textarea
                        ...
                        className="flex-1 text-sm border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-black bg-white resize-none"
                        rows={3}
                        disabled={loading}
                    />
                    <button
